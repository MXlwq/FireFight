...
/*	派遣方案生成*/
	...
	/*public static void Dispatch(String addressInput, 
	...
	public static void Dispatch(
      ...
      /*  if ((fireTypeConfidence != "")&&(fireTypeInput != "")){
			...
			float f = Float.parseFloat(fireTypeConfidence.substring(fireTypeConfidence.indexOf("(")+1, fireTypeConfidence.indexOf(",")));
			float c = Float.parseFloat(fireTypeConfidence.substring(fireTypeConfidence.indexOf(",")+1, fireTypeConfidence.indexOf(")")));
			
			
			
			String sql = "select FireTypeId  from t_firetype where FireTypeName='" + fireTypeInput + "'";
		    ResultSet rs = Dao.executeQuery(sql);
		try {
			while (rs.next()) {
				...
				//NewJFrame.ResultStr = NewJFrame.ResultStr+'\n'+"$事件→[火灾类别"+fireTypeInput+" ] => $事件→("+fireTypeId+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				...
				//插进中间结论表---------
				Integer newId = deathFireLevelid;
				...
				ResultSet rs1 = Dao.executeQuery(sql);
					if (rs1.next()) {
						...
						stmt.executeUpdate("insert into mConclusion(conclusion) values("+newId+")" );
		...
		//areaInput
		//插入输入表---------
		float f = Float.parseFloat(areaConfidence.substring(areaConfidence.indexOf("(")+1, areaConfidence.indexOf(",")));
		float c = Float.parseFloat(areaConfidence.substring(areaConfidence.indexOf(",")+1, areaConfidence.indexOf(")")));
		...
		stmt.executeUpdate("update t_input" +
							" set f="+f+",c="+c+",description = '"+areaInput+"' where input in (" +
				"select AreaId  from t_area where AreaName='" + areaInput + "')");
		//插入输入表结束-------
...
时间：2015.07.17*/
 DecisionIFrame.ResultStr =  DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+areaInput+"]"+"→燃烧面积dao"+ "=> {"+addressInput+"}→"
		...
		float f = Float.parseFloat(newConfidence.substring(newConfidence.indexOf("(")+1, newConfidence.indexOf(",")));
		float c = Float.parseFloat(newConfidence.substring(newConfidence.indexOf(",")+1, newConfidence.indexOf(")")));
		if(conn==null) new Dao();
		Statement stmt = (Statement) conn.createStatement(); 
		stmt.executeUpdate("insert into t_input(input) (" +
				"select VolumeId  from t_volume where VolumeName='" + newInput + "')");
		stmt.executeUpdate("update t_input" +
							" set f="+f+",c="+c+",description = '"+newInput+"' where input in (" +
							"select VolumeId  from t_volume where VolumeName='" + newInput + "')");
		//插入输入表结束-------
		
		String sql = "select FireLevelid,FireLevelName from t_firelevel where VolumeId IN (" +
				"select VolumeId  from t_volume where VolumeName='" + volumeInput + "')";
		ResultSet rs = Dao.executeQuery(sql);
		try {
			while (rs.next()) {
...
DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+volumeInput+"]"+"→燃烧容积Dao"+ "=> {"+addressInput+"}→"
						+"("+volumeFireLevelName+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				
				
				/* ----- 执行插入学习表和相似知识表*/
				sql = "select VolumeId  from t_volume where VolumeName='" + volumeInput + "'";
				rs = Dao.executeQuery(sql);
				int Id = 0;
				while (rs.next()) {
     ...
     // callTimeInput
        ...
        if ((phaseConfidence != "")&&(phaseInput != "")){
			...
			System.out.println("update t_input" +
								" set f="+f+",c="+c+",description = "+newInput+" where input in (" +
								"select StageId  from t_stage where StageName='" + newInput + "')");
			stmt.executeUpdate("update t_input" +
								" set f="+f+",c="+c+",description = '"+newInput+"'  where input in (" +
			...
			String sql = "select StageId  from t_stage where StageName = '" + phaseInput + "'";
			...
			while (rs5.next()) {
				...
				sql = "select FireLevelid,FireLevelName from t_firelevel where StageId = "+Id;
				ResultSet rs = Dao.executeQuery(sql);
		try {
			float f3 = 0, c3 = 0;
			while (rs.next()) {
				...
				System.out.println("StageId:"+Id+"stage:"+phaseFireLevelName);
				//DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+'\n'+"$事件→[阶段"+phaseInput+" ] => $事件→("+phaseFireLevelName+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+phaseInput+"]"+"→所处阶段Dao"+ "=> {"+addressInput+"}→"
						+"("+phaseFireLevelName+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				...
				if (rs1.next()) {
					...
					stmt.executeUpdate("update mConclusion" +
							" set f="+f3+",c="+c3+" where conclusion = "+newId);				
		...
		String sql = "select FireLevelid,FireLevelName from t_firelevel where FireId IN (" +
				"select fireId  from t_fire where fireName='" + situationInput + "')";
		ResultSet rs = Dao.executeQuery(sql);
		try {
			float f3, c3;
			while (rs.next()) {
				...
				DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+situationInput+"]"+"→火势Dao"+ "=> {"+addressInput+"}→"
						+"("+fireFireLevelName+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				
				
				sql = "select fireId  from t_fire where fireName='" + situationInput + "'";
				rs = Dao.executeQuery(sql);
				int Id = 0;
				while (rs.next()) {
					Id = rs.getInt("fireId");
        ...
        if ((spreadConfidence != "")&&(spreadInput != "")){
			...
			Statement stmt = (Statement) conn.createStatement(); 
			stmt.executeUpdate("insert into t_input(input) (" +
					"select SpreadingId  from t_spreading where SpreadingName='" + newInput + "')");
			stmt.executeUpdate("update t_input" +
								" set f="+f+",c="+c+",description = '"+newInput+"' where input in (" +
								"select SpreadingId  from t_spreading where SpreadingName='" + newInput + "')");
			//插入输入表结束-------
			
			String sql = "select SpreadingId  from t_spreading where SpreadingName='" + newInput + "'";
			ResultSet rs5 = Dao.executeQuery(sql);
			int Id = 0;
			while (rs5.next()) {
				Id = rs5.getInt("SpreadingId");
			
			sql = "select FireLevelid,FireLevelName from t_firelevel where SpreadingId = "+ Id;
			ResultSet rs = Dao.executeQuery(sql);
		try {
			float f3 = 0, c3 = 0;
			while (rs.next()) {
				...
				System.out.println("SpreadId:"+Id+"spread:"+spreadFireLevelName);
				//DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+'\n'+"$事件→[蔓延情况"+spreadInput+" ] => $事件→("+spreadFireLevelName+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+spreadInput+"]"+"→蔓延情况Dao"+ "=> {"+addressInput+"}→"
						+"("+spreadFireLevelName+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				//插进中间结论表---------
        ...
        if ((trapedPeopleConfidence != "")&&(trapedPeopleInput != "")){
			...
			Statement stmt = (Statement) conn.createStatement(); 
			stmt.executeUpdate("insert into t_input(input) (" +
					"select TrappedpeopleId  from t_trappedpeople where TrappedpeopleName='" + newInput + "')");
			stmt.executeUpdate("update t_input" +
								" set f="+f+",c="+c+",description = '"+newInput+"' where input in (" +
								"select TrappedpeopleId  from t_trappedpeople where TrappedpeopleName='" + newInput + "')");
			//插入输入表结束-------
			String sql = "select FireLevelid,FireLevelName from t_firelevel where TrappedpeopleId IN (" +
				"select TrappedpeopleId  from t_trappedpeople where TrappedpeopleName='" + trapedPeopleInput + "')";
			ResultSet rs = Dao.executeQuery(sql);
		try {
			float f3=0, c3=0;
			while (rs.next()) {
				...
				DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+trapedPeopleInput+"]"+"→被困人数Dao"+ "=> {"+addressInput+"}→"
						+"("+trapedFireLevelName+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
			
				
				
				sql = "select TrappedpeopleId  from t_trappedpeople where TrappedpeopleName='" + trapedPeopleInput + "'";
				rs = Dao.executeQuery(sql);
				int Id = 0;
				while (rs.next()) {
        ...
        if ((hurtPeopleConfidence != "")&&(hurtPeopleInput != "")){
			...
			Statement stmt = (Statement) conn.createStatement(); 
			stmt.executeUpdate("insert into t_input(input) (" +
					"select InjuredPleopleId  from t_injuredpleople where InjuredPleopleName='" + newInput + "')");
			stmt.executeUpdate("update t_input" +
								" set f="+f+",c="+c+",description = '"+newInput+"' where input in (" +
								"select InjuredPleopleId  from t_injuredpleople where InjuredPleopleName='" + newInput + "')");
			//插入输入表结束-------
			String sql = "select FireLevelid,FireLevelName from t_firelevel where InjuredPeolpeId IN (" +
				"select InjuredPleopleId  from t_injuredpleople where InjuredPleopleName='" + hurtPeopleInput + "')";
			ResultSet rs = Dao.executeQuery(sql);
		try {
			float f3=0, c3=0;
			while (rs.next()) {
				...
				DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+hurtPeopleInput+"]"+"→受伤人数Dao"+ "=> {"+addressInput+"}→"
						+"("+hurtFireLevelName+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				
				
				sql = "select InjuredPleopleId  from t_injuredpleople where InjuredPleopleName='" + hurtPeopleInput + "'";
				rs = Dao.executeQuery(sql);
				int Id = 0;
				while (rs.next()) {
        ...
        if ((deathPeopleConfidence != "")&&(deathPeopleInput != "")){
			...
			Statement stmt = (Statement) conn.createStatement(); 
			stmt.executeUpdate("insert into t_input(input) (" +
					"select DeathTollId  from t_deathtoll where DeathTollName='" + newInput + "')");
			stmt.executeUpdate("update t_input" +
								" set f="+f+",c="+c+",description = '"+newInput+"' where input in (" +
								"select DeathTollId  from t_deathtoll where DeathTollName='" + newInput + "')");
			//插入输入表结束-------
			String sql = "select FireLevelid,FireLevelName from t_firelevel where DeathTollid IN (" +
				"select DeathTollId  from t_deathtoll where DeathTollName='" + deathPeopleInput + "')";
		ResultSet rs = Dao.executeQuery(sql);
		try {
			while (rs.next()) {
				...
				DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+deathPeopleInput+"]"+"→死亡人数Dao"+ "=> {"+addressInput+"}→"
						+"("+deathFireLevelName+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
			
				sql = "select DeathTollId  from t_deathtoll where DeathTollName='" + deathPeopleInput + "'";
				rs = Dao.executeQuery(sql);
				int Id = 0;
				while (rs.next()) {
      ...
      //fireTypeId
		...
		try {
			while (rs.next()) {
				...
				//DecisionIFrame.ResultStr =  DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+fireTypeInput+"]"+"→火灾类别dao"+ "=> {"+addressInput+"}→"
						//+"("+fireTypeInput+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+"\r\n"+"{"+addressInput+"}"+"*"+"["+fireTypeInput+"]"+"→火灾类别"+ "=> {"+addressInput+"}→"
						+"("+fireTypeInput+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
				System.out.println("f="+f+" c="+c);
				/*//插进中间结论表---------
				Integer newId = deathFireLevelid;
...
*/			
        ...
        float c = 0;
        if ((fireTypeConfidence != "")&&(fireTypeInput != "")){
	        f = Float.parseFloat(fireTypeConfidence.substring(fireTypeConfidence.indexOf("(")+1, fireTypeConfidence.indexOf(",")));
			c = Float.parseFloat(fireTypeConfidence.substring(fireTypeConfidence.indexOf(",")+1, fireTypeConfidence.indexOf(")")));
			System.out.println("f="+f+" c="+c);
			f = (float) (f*1.0);
			c = (float) (c*0.9*1.0*f);
        }
		...
		try {
			while (rs.next()) {
				...
				//派遣
				sql = "select * from t_dispatch " +
					  "where TypeId = "+typeId+" AND LevelId = "+LevelId+";";
				...
				try {
					while (rs.next()) {
						...
						String fireTypeDes = "";
						switch (typeId){
						case 21:fireTypeDes="一般性建筑火灾";break;
						case 22:fireTypeDes="高层建筑火灾";break;
						case 23:fireTypeDes="地下空间火灾";break;
						case 24:fireTypeDes="油类火灾";break;
						case 25:fireTypeDes="一般气体火灾";break;
						case 26:fireTypeDes="毒气火灾";break;
						case 27:fireTypeDes="露天堆场火灾";break;
						case 28:fireTypeDes="交通工具火灾";break;
						case 29:fireTypeDes="一般性火灾";break;
						default:fireTypeDes="";
						}
						...
						switch(levelId){
						case 11:fireLevelDes = "一级";break;
						case 12:fireLevelDes = "二级";break;
						case 13:fireLevelDes = "三级";break;
						case 14:fireLevelDes = "四级";break;
						case 15:fireLevelDes = "五级";break;
						default:fireLevelDes = "";
						}
						
						DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+'\n'+"{"+addressInput+"}DAO"+
						"→[火灾类别 "+fireTypeDes+"]+[火灾级别 "+fireLevelDes+"] =>"+"{"+addressInput+"}"+ "→(出动人数:"+FireFighterNum+")+(设备数目:"+Equipment+")（"+df.format(1.0*f)+","+df.format(0.9*c)+"）";	
											
						DecisionIFrame.Plan = DecisionIFrame.Plan+/*"{"+addressInput+"}DAO"+
						"→[火灾类别 "+fireTypeDes+"]+[火灾级别 "+fireLevelDes+"] =>"+"{"+addressInput+"}"+ "→"+*/"" +
								"出动人数:"+FireFighterNum+"\n设备数目:"+Equipment+								
						'\n'+"（Frequency="+df.format(1.0*f)+",Confidence="+df.format(0.9*c)+"）";
						
						Statement stmt = (Statement) conn.createStatement();
						stmt.executeUpdate("insert into t_conclusion values("+conclu+","+f+","+c+")" );
						System.out.println("FireFighterNum="+FireFighterNum+"Equipment="+Equipment);
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
		...
		while (rs.next()){
			...
			f = rs.getFloat("Frequency");
			c = rs.getFloat("Confidence");
			DecisionIFrame.ResultStr = DecisionIFrame.ResultStr+'\n'+ "{$事件}→(["+des1+"]) 火灾) <=>{$事件}→(["+des2+"] 火灾)("+f+","+c+")"        /*"相似性知识："+des1+"+"+des2+"("+f+","+c+")"*/;
		}
			...
			while (rsL1Same.next()){
				...
				if (tc>0.5){
					...
					if (rs1.next()) {
						...
						float f3 = (f1*c1*(1-c2)+f2*c2*(1-c1))/(c1*(1-c2)+c2*(1-c1));
						float c3 = (c1*(1-c2)+c2*(1-c1))/(c1*(1-c2)+c2*(1-c1)+(1-c1)*(1-c2));
						Statement stmt = (Statement) conn.createStatement(); 
						stmt.executeUpdate("update mConclusion" +
								" set f="+f3+",c="+c3+" where conclusion = "+newId);				
	...
	private static void learnAndSimi(Integer cid, int id,String str) throws SQLException {
		...
		//补全新插入t_input的信息
		stmt.executeUpdate("update t_input" +
							" set conclusion = "+cid+" where input = "+id);
		System.out.println("###update t_input" + " set conclusion="+cid+" where input = "+id);
		float f1 = 0,f2=0,c1=0,c2=0;
		String sql2 = "select * from t_input where conclusion ="+cid+" AND input = "+id;
		...
		String sql = "select * from tempLearn where Condition1 = "+id+" or Condition2 = "+id;
		...
		sql = "select * from t_input where conclusion = "+cid +" AND description <> '"+str+"'";
		System.out.println("########sql="+sql);
		ResultSet rs = Dao.executeQuery(sql);
		float f3 = 0,c3=0;
		try {
			while (rs.next()) {				
				...
				String sql4="select * from t_learn where (Condition1 = "+rs.getInt("input")+" and Condition2 = "+id+") or (Condition1 = "+id+" and Condition2 = "+rs.getInt("input")+")";
				...
				if (rs6.next()){
					...
					f3 = (f1*c1*(1-c2)+f2*c2*(1-c1))/(c1*(1-c2)+c2*(1-c1));
					c3 = (c1*(1-c2)+c2*(1-c1))/(c1*(1-c2)+c2*(1-c1)+(1-c1)*(1-c2));
					sql4="update t_learn set Frequency = "+f3+",Confidence = "+c3+" where Condition1 = "+rs6.getInt("Condition1")+" and  Condition2 = "+rs6.getInt("Condition2");
					stmt.execute(sql4);
				}else{
					String sql3 = "insert into t_learn(Condition1,Description1,Condition2,Description2,Conclusion,Frequency,Confidence) " +
							"values("+rs.getInt("input")+",'"+rs.getString("description")+"',"+id+",'"+str+"',"+cid+","+f3+","+c3+")";
		...
		try {
			while (rs.next()) {				
				...
				String des1="";
				String des2="";
				sql = "select FireLevelName from t_firelevel where FireLevelid = "+rs.getInt("conclusion");
				ResultSet rs4 = Dao.executeQuery(sql);
				while (rs4.next()){
					des1 = rs4.getString("FireLevelName");
				}
				sql = "select FireLevelName from t_firelevel where FireLevelid = "+cid;
				...
				String sql4="select * from t_similarity where (Level1 = "+rs.getInt("conclusion")+" and Level2 = "+cid+") or (Level1 = "+cid+" and Level2 = "+rs.getInt("conclusion")+")";
				ResultSet rs6 = stmt.executeQuery(sql4);
				if (rs6.next()){
					f1 = f3;
					c1 = c3;
					...
					f3 = (f1*c1*(1-c2)+f2*c2*(1-c1))/(c1*(1-c2)+c2*(1-c1));
					c3 = (c1*(1-c2)+c2*(1-c1))/(c1*(1-c2)+c2*(1-c1)+(1-c1)*(1-c2));
					sql4="update t_similarity set Frequency = "+f3+",Confidence = "+c3+" where Level1 = "+rs6.getInt("Level1")+" and  Level2 = "+rs6.getInt("Level2");
					stmt.execute(sql4);
				}else{
					String sql3 = "insert into t_similarity(Level1,Description1,Level2,Description2,Frequency,Confidence) values("+rs.getInt("conclusion")+",'"+des1+"',"+cid+",'"+des2+"',"+f3+","+c3+")";
					stmt.executeUpdate(sql3);					
				}
